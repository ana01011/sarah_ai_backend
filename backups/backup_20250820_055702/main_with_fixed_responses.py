...
from relationship_system import relationship_ai
    ...
    try:
        if os.path.exists(MEMORY_FILE):
            with open(MEMORY_FILE, 'r') as f:
                ...
                print(f"📚 Loaded memory for {len(USER_MEMORY)} users", file=sys.stderr)
...
def auto_save_memory():
    ...
    while True:
        time.sleep(60)
        save_memory()
        # # Auto-save happens silently} users", file=sys.stderr)
...
class ChatRequest(BaseModel):
    ...
    agent_role: Optional[str] = "general"  # Added for frontend compatibility
    max_tokens: int = 200
...
async def chat_with_memory(request: ChatRequest):
    ...
    if relationship_context["response_hint"] and relationship_context["score"] < 40:
        ...
        import random
        if False:  # Disabled - always generate natural response
            # Use hint as personality guide instead
            personality_hint = relationship_context["response_hint"]
            prompt = f"Respond in this style: {personality_hint}\n\nUser: {request.message}\nAssistant:"
            response = model(prompt, max_tokens=request.max_tokens, temperature=request.temperature, stop=["User:", "\n\n"], echo=False)
